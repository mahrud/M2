...
CT = new ScriptedFunctor from {
    -- CT^1 or CT^2
    superscript => i -> (
        if i == 1 then cotangentCohomology1 else
        if i == 2 then cotangentCohomology2 else
	error "Higher cotangent cohomology not yet implemented")
    }

----------------------------------------------------------------------------

cotangentCohomology1 = method(
    TypicalValue => Matrix, 
    Options => {
        SourceRing        => null,
        ModuleDeformation => false,
        }
    )

cotangentCohomology1Mod := (I) -> (
     A := quotient I;
     H := Hom(I, A, MinimalGenerators => true);
     H / image transpose(A ** jacobian I))

cotangentCohomology1 Ideal  := opts -> I -> lift(ambient basis(cotangentCohomology1Mod I, SourceRing => opts.SourceRing), ring I)
cotangentCohomology1 Matrix := opts -> F -> (
    if numrows F == 1 and not opts.ModuleDeformation
    then return cotangentCohomology1 ideal F;
...
cotangentCohomology1(List, Matrix) := opts -> (deg, F) -> cotangentCohomology1(deg, deg, F, opts)

cotangentCohomology1(InfiniteNumber, ZZ, Ideal)  :=
cotangentCohomology1(ZZ, InfiniteNumber, Ideal)  :=
cotangentCohomology1(ZZ, ZZ,             Ideal)  := opts -> (lo, hi, I) -> (
    lift(ambient basis(lo, hi, cotangentCohomology1Mod I, SourceRing => opts.SourceRing), ring I))

cotangentCohomology1(InfiniteNumber, ZZ, Matrix) :=
cotangentCohomology1(ZZ, InfiniteNumber, Matrix) :=
cotangentCohomology1(ZZ, ZZ,             Matrix) := opts -> (lo, hi, F) -> (
    if numrows F == 1 and not opts.ModuleDeformation
    then return cotangentCohomology1(lo, hi, ideal F);
    g := map(ambient image F, image F, F);
    ambient basis(lo, hi, coker Hom(g, coker F, MinimalGenerators => true), SourceRing => opts.SourceRing))

----------------------------------------------------------------------------

cotangentCohomology2 = method(
    TypicalValue => Matrix, 
    Options => {
        SourceRing => null,
        }
...
cotangentCohomology2Mod := F -> (
     ...
     R := gens ker F;
     kos := koszul(2, F);
     H := Hom(image R / image kos, A, MinimalGenerators => true);
     J := image(A ** transpose R);
     H / J)

cotangentCohomology2 Ideal  := opts -> I -> cotangentCohomology2(gens I, opts)
cotangentCohomology2 Matrix := opts -> F -> lift(ambient basis(cotangentCohomology2Mod F, opts), ring F)

cotangentCohomology2(ZZ,   Ideal) :=
cotangentCohomology2(List, Ideal) := opts -> (deg, I) -> cotangentCohomology2(deg, gens I, opts)
...
nestedTangentModule := {ModuleDeformation => false, IncludeTrivial => false} >> opts -> (F0X, F0Y) -> (
	...
	if opts.IncludeTrivial then return (
	    ...
	    ker f);
        local jac;
	if numrows F0X > 1 or opts.ModuleDeformation then (
...
nestedObstructionModule := (F0X, F0Y) -> (
     H0 := F0Y//F0X;
     ((Hom(image F0Y, coker F0X))/image((gens Hom(image F0Y, coker F0Y))|transpose (H0**matrix (id_(ZZ^(numrows F0X))))*((gens Hom(image F0X, coker F0X))))))
...
checkTangentSpace(Ideal, Matrix) := (I, N) -> (
    SI := sheaf module I;
    X := Proj ring I;
    numcols N == rank Hom(SI, OO_X(0)/SI))
...
lowestOrderTerms := (G, n, d) -> (
    ...
    lowDeg := apply(d, i -> min({n-2} | positions(G, g -> g^{i} != 0)));
    matrix apply(d, i -> { G_(lowDeg_i)^{i} }))


-- auxiliary function. Returns (A, B) where:
--  A is a matrix of the lowest order terms of the ideal generated by G, 
--  B is a matrix expressing the entries of A in terms of G
...
vecToHom := (v, T, S) -> matrix map(T, S, transpose matrix pack(flatten entries v, numgens T))

--interchanges coefficients with variables
varSwap := R -> (coefficientRing coefficientRing R)[gens R][gens coefficientRing R]

-- FIXME: reduce duplication with correctDeformation
liftDeformation = method(Options => { SanityCheck => true, Verbose => 0, DegreeBound => 0 })
liftDeformation(List, List, List, List) := opts -> (F, R, G, C) -> (
     T := ring F_0;
     n := #F-1; --order so far
     d := numcols C_0; --number of obstructions
     l := numcols R_0; --number of relations
     r := numrows F_0; --number of gens of coker F
     m := numcols F_0;
     if n < 1 then error "Need order at least one";
     if l == 0 then return (
         if n == 1
         then (F|{0*F_0}, R|{0*R_0}, G|{map(T^0, T^1, 0)}, C)
         -- deal with hypersurface case for use with nested deformations
         else (F|{0*F_0}, R|{0*R_0}, G|{0*G_0}, C|{0*C_0}));
     ...
     if n > 1 then ( -- correct coefficients
	     ...
	     M := T^r ** lowG;
	     fixGB M;
             elapsedTime lrelco := lrelco0 // M;
	     NC := -lift(lrelco, T);
             e := numcols lowG;
             -- TODO: use table
	     NCM := matrix apply(e, i -> flatten apply(l, j -> apply(r, k -> NC_(k*e+i, j))));
             NCL := apply(entries(transpose NCM * transpose cm), i -> apply(i, j -> polyToList(j, n)));
     	     CCL := apply(n, i -> matrix apply(NCL, j -> apply(j, k -> k_i)));
	     CO = (CO | {0}) + CCL;
	     );
	 );
     if opts.Verbose > 3 and opts.SanityCheck then print "Doing Sanity Check"; -- TODO: move to sanityCheck
...
-- TODO: reduce duplication with liftDeformation
...
correctDeformation(Sequence, Matrix, List) :=  opts -> (S, M, L) -> (
     ...
     FC := sum(#L, i -> CM_(i, 0)*(L_i_0));
     RC := sum(#L, i -> CM_(i, 0)*(L_i_1));
     FO := drop(F, -1)|{F_n+FC};
     RO := drop(R, -1)|{R_n+RC};
     if opts.Verbose > 3 and opts.SanityCheck then print "Doing Sanity Check"; -- TODO: move to sanityCheck
...
firstOrderDeformations(Matrix, Matrix, Matrix) :=  opts -> (F, R, T1) -> (
    local T;
    if T1 == 0 then return (
	 T = (ring F)(monoid[Join => false]);
	 {sub(F, T), sub(0*F, T)}, {sub(R, T), sub(0*R, T)}); -- if rigid, nothing to do
     n := numcols T1; --number of deformation parameters
     t := getSymbol opts.DefParam; --deformation parameter name
     T  = (ring F)([t_1..t_n, Join => false, Degrees => (apply((degrees T1)_1, i -> -1*i))]); --setup ring with parameters
     FO := {substitute(F, T), sum(n, i -> T_i * sub(vecToHom(T1_{i}, target F, source F), T))}; --first order family
...
versalopts2 := versalopts3 | { SmartLift => true, CorrectionMatrix => "auto", CacheName => null }
versalopts  := versalopts2 | { DefParam => defaultdefparam, CacheName => null }
...
versalDeformation Matrix :=  versalopts >> opts -> F0 -> (
     ...
     if (numrows F0)>1 then return versalDeformation(F0, CT^1(F0), extMatrix(F0), opts);
     versalDeformation(F0, CT^1(F0), CT^2(F0), opts)
     )

localHilbertScheme Matrix :=  opts -> F0 -> (
     if (numrows F0)>1 then error "Input should only have one row.";
     if opts.Verbose > 0 then print "Calculating first order deformations and obstruction space";
...
versalDeformation(Matrix, Matrix, Matrix) := versalopts >> opts -> (F0, T1, T2) -> (
     ...
     NT2 := T2;
     if not T2 == 0 then (
         RACTION := map(A^l, T^(r*m), Hom(R_0, A));
         NT2 = matrix (map(A^l, sub(source T2, T), sub(T2, T)) % RACTION));
     C := {sub(NT2, T)};
     G := {};
     if (numcols T1)=== 0 then (print "No deformation parameters!");
     -- TODO: just pass opts?
     versalDeformation(F, R, G, C, 
         HighestOrder     => opts.HighestOrder, 
         DegreeBound      => opts.DegreeBound, 
         Verbose          => opts.Verbose, 
         SanityCheck      => opts.SanityCheck, 
         PolynomialCheck  => opts.PolynomialCheck, 
         SmartLift        => opts.SmartLift, 
         CorrectionMatrix => opts.CorrectionMatrix, 
         CacheName        => cachename)
     )
...
versalDeformation(List, List, List, List) := versalopts2 >> opts -> (f, r, g, c) -> (
     ...
     while i < ord do (
	  ...
	  (F, R, G, C) = liftDeformation(F, R, G, C, -- ~45% of the computation
              Verbose     => opts.Verbose, 
              SanityCheck => opts.SanityCheck, 
              DegreeBound => opts.DegreeBound);
  	 if opts.SmartLift and numcols C_0 > 0 then
		(F, R) = correctDeformation((F, R, G, C), M, L, 
                    Verbose     => opts.Verbose, 
                    SanityCheck => opts.SanityCheck);
	  i = i+1;
...
nestedopts := {
    Verbose     => 0, 
    SanityCheck => true, 
    DefParamX   => defaultdefparamX, 
    DefParamY   => defaultdefparamY, 
    }
...
setupNestedDeformation(List) := opts -> L -> (
     ...
     if #L == 6 then  (L1 := toList(0..((numcols F0X)*(numrows F0X))-1); (T1X, T1Y)=(submatrix(T1XY, L1, ), submatrix'(T1XY, L1, )));
     ...
     t2Y := numcols T2Y;
     t2XY := numcols T2XY;
     CX ={CX_0|(matrix map(cRing2^(numrows T2X), cRing2^(t2Y+t2XY), (i, j) -> 0))};
     CY ={(matrix map(cRing2^(numrows T2Y), cRing2^(t2X), (i, j) -> 0))|CY_0|(matrix map(cRing2^(numrows T2Y), cRing2^(t2XY), (i, j) -> 0))};
     CXY ={(matrix map(cRing2^(numrows T2XY), cRing2^(t2X+t2Y), (i, j) -> 0))|CXY_0};
    ...
    Xperturb := sub(transpose (H0**matrix (id_(ZZ^(numrows F0X))))*((gens Hom(image F0X, coker F0X))), cRing2); --rels from F0X def
...
liftDeformation NestedDeformation := opts -> D -> (
     ...
     G := D#"G"|{GX_-1+GY_-1};
     d := numcols D#"CXY"_0; --number of obstructions
     r := numrows D#"FX"_0; --number of gens of coker F
     -- find lowest order terms of obstruction equations
     ...
     fterms := -FY_(n+1)+sum(1..n+1, i -> FX_i*D#"H"_(n+1-i)); --order n+1 terms
     eterms := if n<2 then 0 else transpose matrix pack(r, flatten entries (sum(n-2+1, i -> D#"CXY"_i*G_(n-1-i))));
    -- eterms := sum(1..(n-2), i -> D#"CXY"_i*G_(n-1-i)); -- terms from base equations
     rem := homomorphism' sub(fterms+eterms, QT);
     if opts.Verbose > 3 then print "Calculating Joint Obstruction Equations";
     T2basis := D#"T2basis";
     ng := apply({D#"Xperturb", D#"Yperturb", D#"O1"}, i -> numcols i);
	 ...
	 e := numcols lowG;
	 NCM := matrix apply(e, i -> flatten apply(numcols NC, j -> apply( r, k -> ((NC)_(k*e+i, j)))));
	 NCL := apply(entries (transpose NCM*transpose  cm), i -> apply(i, j -> polyToList(j, n)));
	 CCL := -apply(n, i -> matrix apply(NCL, j -> apply(j, k -> k_i)));
...
nestedHilbertScheme(Matrix, Matrix) := opts -> (F0X, F0Y) -> (
    ...
    D := setupNestedDeformation({F0X, T2X, F0Y, T2Y, T1XY, T2XY}, 
        Verbose     => opts.Verbose, 
        SanityCheck => opts.SanityCheck, 
	DefParamX   => opts.DefParamX, 
        DefParamY   => opts.DefParamY);
    versalDeformation(D, 
        Verbose         => opts.Verbose, 
        SanityCheck     => opts.SanityCheck, 
        HighestOrder    => opts.HighestOrder, 
...
sanityCheck(ZZ, NestedDeformation) := (n, D) -> (
    ...
    r := numrows D#"FX"_0;
    --
    if n >  1 and not 0 == sum(n+1, i -> D#"FX"_i * D#"H"_(n-i)) - D#"FY"_n + transpose matrix pack(r, flatten entries sum(n-2+1, i -> D#"CXY"_i * D#"G"_(n-2-i)))
    then error "Nested check failed";
    if n == 1 and not 0 == sum(n+1, i -> D#"FX"_i * D#"H"_(n-i)) - D#"FY"_n
    then error "Nested check failed. Consider adjusting DegreeBound.";
    --
    if not 0 == transpose flatten sum(n+1, i -> D#"FX"_i * D#"RX"_(n-i)) + sum(n-2+1, i -> D#"CX"_i * D#"G"_(n-2-i))
    then error "Check for X failed. Consider adjusting DegreeBound.";
    if not 0 == transpose flatten sum(n+1, i -> D#"FY"_i * D#"RY"_(n-i)) + sum(n-2+1, i -> D#"CY"_i * D#"G"_(n-2-i))
    then error "Check for Y failed. Consider adjusting DegreeBound.";
    --
    true)
...
sanityCheck(ZZ, List) := (n, L) -> (
    ...
    if not 0 == transpose flatten sum(n+1, i -> F_i * R_(n-i)) + sum(n-1, i -> C_i * G_(n-2-i))
    then error "Sanity check failed. Consider adjusting DegreeBound.";
    true)
...
isPolynomial NestedDeformation := D -> (
    isPolynomial(D#"FX", D#"RX", D#"G", D#"CX") and isPolynomial(D#"FY", D#"RY", D#"G", D#"CY")
    and 0 == homomorphism'(sum D#"FX" * sum D#"H" - sum D#"FY") + sum D#"CXY" * sum D#"G")
...
assert( numcols normalMatrix(0, gens J)== 24)
...
restart
elapsedTime check needsPackage "VersalDeformations"
