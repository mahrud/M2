...
-- FIXME: this breaks S = CC[x,y]; trim coker matrix {{x,y},{-y,x}}
-- also see the tests in tests/normal/isPromotable.m2
...
Hom(CoherentSheaf, CoherentSheaf) := Module => opts -> (F, G) -> (
    ...
    F' := prune F;
    G' := prune G;
    z := degree 1_(ring variety F);
    H := prune sheafHom(F', G', opts, DegreeLimit => z);
...
homomorphism' SheafMap := o -> h -> map(Hom(source h, target h, o), ,
    part(degree 1_(ring variety h), homomorphism'(matrix h, o)))
...
prune SheafMap := minimalPresentation SheafMap := SheafMap => opts -> f -> f.cache#(symbol minimalPresentation => opts) ??= (
