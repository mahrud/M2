...
lowestOrder := (G,F,C,nk) -> (
    ...
    GBcalc := gb(I,DegreeLimit => dl,ChangeMatrix => true); -- ~20% of the computation
...
liftDeformation(List, List, List, List) := opts -> (F, R, G, C) -> (
     ...
     if l == 0 then return (
         if n == 1
         then (F|{0*F_0},R|{0*R_0},G|{map((ring F_0)^0,(ring F_0)^1,0)},C)
	 ...
	 if lowG == 0 then CO =(CO|{0*CO_0})
	     ...
	     NCM := matrix apply(e,i -> flatten apply(l,j -> apply( r, k -> ((NC)_(k*e+i,j)))));
             -- TODO: ~50% of time is in polyToList
...
firstOrderDeformations(Matrix,Matrix,Matrix) :=  opts -> (F,R,T1) -> (
    ...
    if T1 == 0 then return (
     ...
     FO := {substitute(F,T),sum(n,i -> (value defparam)_(i+1)*sub(vecToHom(T1_{i},target F,source F),T))}; --first order family
