...
-- cohomology with many twists, the functions hh^i(F,b1,b2) or hh^i(F(*))
...
-- The function HH^p(F(>=b)) computes the cohomology of a coherent sheaf on a closed subspace
...
-- hh^p(F, b1, b2) or hh^p(F(*)) will probably be faster. The base ring of the given module
-- should be a positively graded ring, not necessarily a graded polynomial ring.
...
-- the line bundles O(c) on the stack, which ensures that H^i(X,M~(c)) = H^i(X,M(c)~).
--
-- In more detail, HH^p(F(>=b)) computes _at least_ H^p(X, F(>=b)) as a module.
-- If H^p(X, F(*)) is bounded below, we compute it completely, regardless of the given number b.
-- Also, if H^0(X, F(*)) is not bounded below, we compute a module that is correct in degrees at least b
-- and that also has a map of graded modules from M to the output module. The map is stored
...
Ext(ZZ, CoherentSheaf, SumOfTwists) := Module => opts -> (m, F, S) -> (
    ...
    -- Here e is the degree at which M was truncated, which
    -- may be used later for computing the Yoneda extension.
