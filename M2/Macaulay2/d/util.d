...
export toExpr(x:RawMatrixStringPair):Expr := seq(Expr(RawMatrixCell(x.a)),toExpr(x.b));
export toExpr(x:RawMatrixStringPairOrNull):Expr := when x is p:RawMatrixStringPair do toExpr(p) is null do engineErrorMessage();
export toExpr(x:RawMatrixStringPairArray):Expr := Expr( list( new Sequence len length(x) do foreach p in x do provide toExpr(p) ) );
export toExpr(x:RawMatrixStringPairArrayOrNull):Expr := when x is r:RawMatrixStringPairArray do toExpr(r) is null do engineErrorMessage();
