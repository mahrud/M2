                                                      ...
                                                      const Matrix *grading)
{
  try
    {
      const Ring *R = C->get_ring();
      if (R != grading->get_ring())
        {
          ERROR("expected both matrices to be over the same ring");
          return nullptr;
        }

      const size_t c = C->n_cols();  // rank of ambient lattice
      const size_t r = C->n_rows();  // number of cone rays
      if (grading->n_rows() != 1 || grading->n_cols() != c)
        {
          ERROR("expected grading to be a 1 x n matrix matching the ray matrix");
          return nullptr;
        }
      ...
      auto grade = libnormaliz::Matrix<Integer>(1, c);
      for (size_t j = 0; j < c; j++)
        grade[0][j] = static_cast<Integer>(grading->elem(0, j).get_mpz());

      libnormaliz::InputMap<Integer> input;
      input[libnormaliz::Type::cone] = rays;
      input[libnormaliz::Type::grading] = grade;

      auto cone = libnormaliz::Cone<Integer>(input);
      cone.compute(libnormaliz::ConeProperty::HilbertQuasiPolynomial);

      const auto& hs = cone.getHilbertSeries();
      const auto& qp = hs.getHilbertQuasiPolynomial();
      const mpz_class denom = hs.getHilbertQuasiPolynomialDenom();
      const long shift = hs.getShift();
      const long period = hs.getPeriod();

      size_t max_coeffs = 3;  // denominator, period, shift
      for (const auto& constituent : qp)
        if (constituent.size() > max_coeffs)
          max_coeffs = constituent.size();

      MatrixConstructor mat(R->make_FreeModule(qp.size() + 1), max_coeffs);
      for (size_t i = 0; i < qp.size(); i++)
        for (size_t j = 0; j < qp[i].size(); j++)
          mat.set_entry(i, j, makeIntegerRingElem(qp[i][j]));

      // Final row stores shared metadata: denominator, period, shift.
      mat.set_entry(qp.size(), 0, makeIntegerRingElem(denom));
      mat.set_entry(qp.size(), 1, makeIntegerRingElem(mpz_class(period)));
      mat.set_entry(qp.size(), 2, makeIntegerRingElem(mpz_class(shift)));
