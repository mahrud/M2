...
CoherentSheaf == Module        := Boolean => (F, M) -> F == sheaf(variety F, M)
Module        == CoherentSheaf := Boolean => (M, F) -> F == M
...
cotangentSheaf ProjectiveVariety := opts -> X -> X.cache#(symbol cotangentSheaf => opts) ??= (
    -- compute the cotangent sheaf of a closed subscheme of a projective scheme over a base ring.
    --
    -- For example, if the coarse moduli space of X is quasi-smooth and well-formed,
    -- then the pushforward of this sheaf to the coarse moduli space is the sheaf
    -- of reflexive differentials. If X is normal but not quasi-smooth,
    -- use reflexiveDifferentials instead for the sheaf of reflexive differentials.
    --
    ...
    --
    -- Say X = Proj R is a closed subspace of a weighted projective stack Proj S,
    -- so R is the quotient of S by the ideal(f_1,...,f_r) over S.
    R := assertWeightedZZGraded ring X;
    -- d: R^n -> R sends the basis element dx_i to a_i x_i, for 0 <= i <= n-1.
    d := map(R^1, , { apply(gens R, flatten degrees R, times) });
    assert isHomogeneous d;
    -- e: R^r -> R^n sends the basis element df_j to sum_{i=0}^(n-1) df_j / dx_i dx_i.
    e := jacobian presentation R ** R;
...
cotangentSheaf AffineVariety := opts -> X -> X.cache#(symbol cotangentSheaf => opts) ??= (
    -- compute the cotangent sheaf of an affine scheme over a base ring
    -- Say X = Spec R is a subscheme of the affine space Spec S,
    -- so R is the quotient of S by the ideal(f_1,...,f_r) over S.
    R := ring X;
    n := numgens R; -- So Y = Spec S has dimension n.
    -- e: R^r -> R^n sends the basis element df_j to sum_{i=1}^n df_j/dx_i dx_i.
    e := jacobian presentation R ** R;
    prune' := if opts.MinimalGenerators then minimalPresentation else identity;
    prune' sheaf(X, cokernel e))
...
tangentSheaf Variety := opts -> X -> (
    prune' := if opts.MinimalGenerators then minimalPresentation else identity;
    sheaf(X, prune' module dual cotangentSheaf(X, opts)))

idealSheaf = method(TypicalValue => CoherentSheaf)
idealSheaf Variety := X -> sheaf(ambient X, ideal (ring X).relations)

canonicalSheaf = method(TypicalValue => CoherentSheaf, Options => options cotangentSheaf)
canonicalSheaf ProjectiveVariety := opts -> X -> (
    R := assertWeightedZZGraded ring X;
    ...
    omega1 := if isStandardGraded R then cotangentSheaf(X, opts) else (naiveCotangentComplex(X, opts))_0;
    prune' := if opts.MinimalGenerators then minimalPresentation else identity;
    sheaf(X, prune' module dual dual determinant(omega1, Strategy => opts.Strategy)))

canonicalSheaf AffineVariety := opts -> X -> (
    omega1 := cotangentSheaf(X, opts);
    prune' := if opts.MinimalGenerators then minimalPresentation else identity;
    sheaf(X, prune' module dual dual determinant(omega1, Strategy => opts.Strategy)))
...
reflexiveDifferentials(ZZ, Variety) := opts -> (i, X) -> (
    prune' := if opts.MinimalGenerators then minimalPresentation else identity;
    sheaf(X, prune' module dual dual exteriorPower(i, cotangentSheaf(X, opts), Strategy => opts.Strategy)))

dualizingSheaf = method(TypicalValue => CoherentSheaf, Options => options cotangentSheaf)
dualizingSheaf ProjectiveVariety := opts -> X -> (
    -- For an n-dim scheme X of finite type over a field k, the dualizing complex
    -- omega_X^{\bullet} = f^!(O_(Spec k)) is concentrated in cohomological degrees at >= -n.
    -- We compute here H^(-n)(omega_X^{\bullet}), known as the dualizing sheaf of X.
    -- Its support is the union of the n-dimensional irreducible components of X.
    -- The output is also reasonable for X a closed subspace of a weighted projective stack.
    R := assertWeightedZZGraded ring X;
    ...
    prune' := if opts.MinimalGenerators then minimalPresentation else identity;
    sheaf(X, prune'(Ext^(n - dim X - 1)(cokernel presentation R, w) ** R)))

dualizingSheaf AffineVariety := opts -> X -> (
    R := ring X;
    S := ring presentation R; -- This is a polynomial ring.
    n := numgens S; -- dim Spec S
    prune' := if opts.MinimalGenerators then minimalPresentation else identity;
    sheaf(X, prune'(Ext^(n - dim X)(cokernel presentation R, S) ** R)))
